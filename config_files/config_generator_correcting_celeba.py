...
base_config = {
    ...
    'artifacts_file': 'data/artifacts_celeba.json',
...
def store_local(config, config_name):
    ...
    config['ckpt_path'] = f"{local_config['checkpoint_dir']}/checkpoint_{model_name}_last_celeba.pth"

    config['ckpt_path_corrected'] = f"{local_config['checkpoint_dir']}/{config_name}/last.ckpt"
...
def store_cluster(config, config_name):
    model_name = config['model_name']
    config['ckpt_path'] = f"checkpoints/checkpoint_{model_name}_last_celeba.pth"
...
for model_name, layer_name in [
    ...
    if model_name == "efficientnet_b0":
        base_config['dataset_name'] = "celeba_biased"
    elif model_name == "resnet18":
        base_config['dataset_name'] = "celeba_biased"
    elif model_name == "vgg16":
        base_config['dataset_name'] = "celeba"
    ...
    for artifact in [
        ...
        for lr in [
            0.00002 if model_name == "resnet18" else 0.0001,
        ]:
            base_config['lr'] = lr
