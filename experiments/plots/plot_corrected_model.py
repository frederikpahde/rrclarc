...
def plot_corrected_model(config):
    ...
    ckpt_path_corrected = config['ckpt_path_corrected']
    ...
    if config['normalized'] == 'max':
        max = attr_corrected.heatmap.flatten(start_dim=1).max(1, keepdim=True).values[:, None]
    elif config['normalized'] == 'abs_max':
        max = attr_corrected.heatmap.flatten(start_dim=1).abs().max(1, keepdim=True).values[:, None]
    else:
    ...
    attr_corrupted = attribution_corrupted(data.requires_grad_(), condition_corrupted, composite)

    if config['normalized'] == 'max':
        max = attr_corrupted.heatmap.flatten(start_dim=1).max(1, keepdim=True).values[:, None]
    elif config['normalized'] == 'abs_max':
        max = attr_corrupted.heatmap.flatten(start_dim=1).abs().max(1, keepdim=True).values[:, None]
    else:
        raise ValueError("Unknown normalization")

    heatmaps_corrupted = attr_corrupted.heatmap / max
